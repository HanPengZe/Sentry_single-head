...
//    PositionPID(float _a_p, float _b_p, float _c_p,float _a_i,float _b_i,float _c_i) : a_p(_a_p), b_p(_b_p), c_p(_c_p), a_i(_a_i),b_i(_b_i),c_i(_c_i){}
    void SetPIDParam(float _Kp, float _Ki, float _Kd, float _I_Term_Max, float _Out_Max,float _Dt)
    {
		...
		void SetPidChange(float _a_p,float _b_p,float _c_p,float _a_i,float _b_i,float _c_i)
		{
			a_p  = _a_p;
			b_p  = _b_p;
			c_p  = _c_p;
