...
void Robot_classdef::RefereeMsg_Write(uint16_t id, uint8_t can_rx_data[])
{
    ...
    case 0x341:
        ...
        Shoot.BulletSpeed = *speed;
				...
				//当前热量
				shooter_id1_17mm_heat = can_rx_data[5]<<8|can_rx_data[4];
//				Infantry.shooter_barrel_heat_limit = can_rx_data[5];
...
//				shooter_barrel_heat_limit =can_rx_data[9]<<8|can_rx_data[8];
			...
			case 0x342:
				Vision.Radar_Send_Msg.Pack.game_progress = can_rx_data[0];  //模拟使用，记得恢复
//			Vision.Radar_Send_Msg.Pack.game_progress = 4;//模拟使用，记得恢复
...
//				}
			...
			case 0x343:
				Vision.Radar_Send_Msg.Pack.game_progress_remain = can_rx_data[0]<<8 | can_rx_data[1]; //模拟使用，记得恢复
//			Vision.Radar_Send_Msg.Pack.game_progress_remain = 8888;//模拟使用，记得恢复
#if SendRadar_Mode ==RMUC
				...
				Vision.Radar_Send_Msg.Pack.defense_hp = can_rx_data[6]<<8 | can_rx_data[7];
#elif SendRadar_Mode == RMUL
				Vision.Radar_Send_Msg.Pack.func = can_rx_data[6];//模拟使用，记得恢复
//			Vision.Radar_Send_Msg.Pack.func =1;//模拟使用，记得恢复
				
#endif

			break;
			case 0x345:
				shooter_barrel_heat_limit =can_rx_data[1]<<8|can_rx_data[0];
				 

				uint8_t a=can_rx_data[2]=can_rx_data[3]=can_rx_data[4]=can_rx_data[5]=can_rx_data[6]=can_rx_data[7];
