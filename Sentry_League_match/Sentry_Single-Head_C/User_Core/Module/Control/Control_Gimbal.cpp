...
//		YawPID[C_IMU][PID_Outer].SetPidChange(0.019f,0.40f,0.030f,0.0f,0.0f,0.0f);
    YawPID[C_IMU][PID_Inner].SetPIDParam(1150.0f, 570.0f, 0.0f, 10000, GM6020_MAX_OUT, 0.002f);//
...
//		YawPID[V_CIMU][PID_Outer].SetPidChange(0.054f,0.6400f,0.0050f,0.0f,0.0f,0.0f);
    ...
    YawPID[V_CIMU][PID_Inner].SetPIDParam(800.0f, 200.0f, 0.0f, 13000, GM6020_MAX_OUT, 0.002f);//380 580  12000
    ...
    PitPID[C_IMU][PID_Outer].I_SeparThresh = 500;
    PitPID[C_IMU][PID_Inner].SetPIDParam(550.0f, 400.0f, 0.0f, 5000, GM6020_MAX_OUT, 0.002f);//500 600
    PitPID[C_IMU][PID_Inner].I_SeparThresh = 100;


    /* Pit 自瞄 C板IMU */
    PitPID[V_CIMU][PID_Outer].SetPIDParam(0.69f, 0.0f, 0.0f, 2000, 15000, 0.002f);
    PitPID[V_CIMU][PID_Outer].I_SeparThresh = 500;
    PitPID[V_CIMU][PID_Inner].SetPIDParam(500.0f, 400.0f, 0.0f, 8000, GM6020_MAX_OUT, 0.002f);//500 600
    PitPID[V_CIMU][PID_Inner].I_SeparThresh = 200;
#endif
...
{		
	Vision.Tar_Yaw_Vision = (Gimbal.Get_IMUTotalAngle(Yaw)+ Sentry_Recv_Msg_usb.Pack.yaw)*ENCODER_ANGLE_RATIO;
	Vision.Tar_Pit_Vision = (Gimbal.Get_IMUAngle(Rol)+ Sentry_Recv_Msg_usb.Pack.pit)*ENCODER_ANGLE_RATIO;//在总角度的基础上加上视觉给的偏差值
...
//						Vision.KalmanFilter_Ctrl(VISION_ON, Get_TargetAngle(Yaw), Get_TargetAngle(Pit));
				...
				else//没有敌人
				{
							...
							Target.CIMU_Pit = GIMBAL_PIT_CENTRE - pSinVal*60;
...
//						}
    ...
    {
			if(Vision.Radar_Recv_Msg.Pack.chassis_lx == 0 && Vision.Radar_Recv_Msg.Pack.chassis_ly == 0 )
			{}
			else
			{
    ...
    /*--- Pit 角度限制 ---------------------------------------------*/
    Pit_AngleLimit();
		if(hit==0&&Vision.Get_Enemy() == false)//哨兵死后Yaw轴目标值还在累加，比赛结束后上电云台开始疯转，条件是在没有敌人和没被击打的情况下才限幅
