...
#endif
    /*--- 跟随模式 PID -----------------------------------------------------------------------*/
    Follow_PID[PID_Outer].SetPIDParam(1000.0f/* 4500.0f */, 100.0f, 0.0f, 2000, 4000, 0.002f);//7200
    Follow_PID[PID_Outer].I_SeparThresh = 1000;
    Follow_PID[PID_Inner].SetPIDParam(0.75f, 0.0f, 0.0f, 2000, 5000, 0.002f);  //0.45  0.1
...
//			float coe[4] = {0.0}; //系数	动态分配数组内存。每个电机的占用电流之比的系数
//			PowerLimit.SumCurrent_IN = PowerLimit.SumCurrent_Out = 0;//首先清空之前的总电流，重新进行计算当前总电流
////			PowerLimit.SumCurrent_IN = Referee.PowerHeatData.chassis_current;
//			for (int i = 0; i < 4; i++)
//			{
//					PowerLimit.SumCurrent_IN += abs(DRV_Motor[i].getGivenCurrent());
//			}

//			for (int i = 0; i < 4; i++)
//			{
//					coe[i] = ((float)(DRV_Motor[i].getGivenCurrent())) / ((float)(PowerLimit.SumCurrent_IN));//占流比=当前电机的电流/当前总电流
//			}

//			power_buffer=Get_PowerBuffer();//缓冲热量大小
//			int16_t powerBuffErr =Get_PowerLimit() - power_buffer; //用掉的缓冲能量=缓冲功率上限-剩余的缓冲功率
//			if(powerBuffErr<Get_PowerLimit())//开始使用缓冲功率
//			{
//				PowerLimit.powerBuffRatio = (float)power_buffer /Get_PowerLimit(); //缓冲功率使用比=剩余功率/最大缓冲功率
//				PowerLimit.SumCurrent_Out = PowerLimit.SumCurrent_IN * PowerLimit.powerBuffRatio; //输入的总电流*缓冲功率使用比这样做的目的是根据缓冲功率的利用情况来调整输出的总电流。如果缓冲功率完全未被使用，输出的总电流将等于输入的总电流；而如果缓冲功率完全被使用，输出的总电流将等于输入的总电流乘以一个小于或等于1的比例因子。
//			}
//			else if(powerBuffErr==Get_PowerLimit())
//			{
//				PowerLimit.SumCurrent_Out= PowerLimit.SumCurrent_IN;
//			}
//			for (int i = 0; i < 4; i++)
//			{
//				if(isnan(PowerLimit.SumCurrent_Out * coe[i])!=1)
//				{
//					DRV_Motor[i].Out = (float)(PowerLimit.SumCurrent_Out * coe[i]);//分配后当前电机所需电流=总的电流*当前电机占流比
//				}
//			}
