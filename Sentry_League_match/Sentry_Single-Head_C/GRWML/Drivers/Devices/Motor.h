...
protected:
    ...
    void update_angle(uint8_t can_rx_data[])
    {
      ...
      total_encoder = round_cnt * 8192 + encoder - encoder_offset;//Gimbal.Motor[Yaw].encoder_offset = GIMBAL_YAW_CENTRE;，此时为偏差总角度=总角度-中心角度即为偏离中心角度的编码器值

      angle = total_encoder / ENCODER_ANGLE_RATIO();//此时角度为偏差角度
    }
