...
//	 		Auto.God_Detection();
...
#if SendRadar_Mode == RMU_C
		
#elif SendRadar_Mode == RMU_L
			Auto.Move_Point();
#endif
    ...
    RefereeMsg_Send_HP(RefereeHpData);
		RefereeMsg_Send_Data3(RefereeData3);
...
{  
    ...
    /*---------------------------修改部分
    //当前热量下的可发射数量
		if(Referee.GameRobotState.shooter_barrel_heat_limit<Get_SentryGunHeat1()){data[4]=100;}
		else{data[4]=(uint8_t)(Referee.GameRobotState.shooter_barrel_heat_limit-Get_SentryGunHeat1())/10;}
		if(Referee.GameRobotState.shooter_barrel_heat_limit<Get_SentryGunHeat2()){data[5]=100;}
		else{data[5]=(uint8_t)(Referee.GameRobotState.shooter_barrel_heat_limit-Get_SentryGunHeat2())/10;}
		*/
//    data[4] = (uint8_t)(24-Get_SentryGunHeat1()/10);
//    data[5] = (uint8_t)(24-Get_SentryGunHeat2()/10);

	//--------------------------------改为：
		data[4] = Get_SentryGunHeat1();
		data[5] = Get_SentryGunHeat1()>>8;
   ...
   data[6] = Auto.HurtArmor_NoZero;
    ...
    data[7] |= Referee.GameRobotState.power_management_shooter_output == ON?1<<6:0;
...
//		data[8]	 = Referee.GameRobotState.shooter_barrel_heat_limit;//低八位
//		data[9]  = Referee.GameRobotState.shooter_barrel_heat_limit>>8;//高八位
...
{   
    ...
    data[1] = ParmData;
    //自身剩余血量
    ParmData = Referee.GameRobotState.current_HP;
    data[2] = ParmData>>8; //--- 
    data[3] = ParmData;
    //17mm 弹丸允许发弹量
...
void Robot_classdef::RefereeMsg_Send_Data3(uint8_t *data)
{
    //--- 0x345
    uint16_t ParmData;
    ...
    data[2]  = 0;
		...
		data[6]  = 0;
		data[7]  = 0;
	  CANx_SendData(&hcan1,0x345,data,8);
...
{
    return Referee.PowerHeatData.shooter_id1_17mm_heat;//枪口热量
}
